...
- **Backend:** Use `console.log` sparingly; prefer structured logging as in [logging.md](logging.md). Run server with `bun run dev` (watch mode) for quick iteration.
...
1. Do not remove or break the existing run scripts (`bun run dev`, `bun run server`, `bun run build`) when adding or changing code.
